-- DOWNLOADED FROM https://raw.githubusercontent.com/siffiejoe/lua-testy/master/src/testy.lua
...
local function evaluate_test_assertion(finfo, cinfo, ok, ...)
  ...
  if do_tap then
    ...
    -- consumer.
    if type((...)) == "string" and ((...):match("^#[\t ]*[Tt][Oo][Dd][Oo]") or (...):match("^#[\t ]*[Ss][Kk][Ii][Pp]")) then
      fh:write(" ", (...))
    end
    -- In case the test failed, an additional diagnostic message is
    -- printed:
    if not ok then
      local msg = (...) ~= nil and tostring((...)) or "test assertion failed!"
      fh:write("\n# Failed test (", src, " at line ", line, ": '", msg:gsub("\n", "\n#\t"), "')")
    end
    fh:write("\n")
  else
...
local function line_ret_hook(event, no)
  if event ~= "tail_return" and main_chunk(2) then
    ...
    if event == "line" then
      ...
      while name do
        if #name >= #prefix and type(value) == "function" and name:sub(1, #prefix) == prefix then
          locs[#locs + 1] = {
            caption = name:sub(#prefix + 1):gsub(
              "_+",
              function(u)
  ...
  if not ok then
    ...
    if not do_tap then
      ...
      for _, f in ipairs(t) do
        fh:write("[FAIL] ", t.source, ":", f.line, ": in function '", t.name, "'\n")
        if f.reason then
